...
async def require_file(message: types.Message, state: FSMContext):
    await message.answer("Отправьте документ", reply_markup=get_simple_keyboard())
...
async def require_number_of_copies(message: types.Message, state: FSMContext):
    await message.answer(
        "Напишите в чат необходимое вам количество " "копий документа",
...
async def get_file(message: types.Message, state: FSMContext):
    ...
    if not number_of_pages:
        return await message.answer(
            "Ваш файл не валиден. Я принимю только pdf " "с форматом A4."
        )
...
async def require_printing_mode(message: types.Message, state: FSMContext):
    await message.answer("Выберите режим печати", reply_markup=get_printing_method_kb())
...
async def printing_mode(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"""Стоимость заказа составляет {coast} рублей. 
...
async def pay_online(message: types.Message, state: FSMContext):
    ...
    for index in range(len(numbers)):
        msg += (
            "+7" + str(numbers[index]) + (" или " if index != len(numbers) - 1 else "")
        )
...
async def pay_cash(message: types.Message, state: FSMContext):
    await message.answer(
        "Ваш заказ принят к ожиданию. Ожидаем с наличными " "в комнате 254.",
...
async def pay_card(message: types.Message, state: FSMContext):
    ...
    id_ = await database.TransactionDB().create_transaction(task.id_, task.user_id)
    url = payment.create_pay_link(id_, task.coast)
    await message.answer(
        f"Для начала печати оплатите по ссылке {url}",
        reply_markup=get_card_keyboard(id_, database.TaskType.PRINT_TASK, task.id_),
    )
    await message.answer(
        text="После оплаты нажмите на кнопку 'проверить'",
        reply_markup=get_main_keyboard(),
    )


async def finish_creation(
    message: types.Message, state: FSMContext, pay_way: database.PayWay
) -> database.Task:
    data = await state.get_data()
    task = database.Task(
        data["id"],
...
async def print_task_information_to_admins(task: database.Task):
    for admin_id in Shift().get_active():
        await bot.send_message(
            admin_id,
            f"Новый заказ сканирования №: {task.id_}."
...
@router.callback_query(
    and_f(
        CardCallback.filter(F.action == Actions.ACCEPT),
...
async def check_transaction(callback: CallbackQuery, callback_data: CardCallback):
    for admin_id in Shift().get_active():
        await bot.send_message(
            admin_id,
            f"СРОЧНО!. Оплачен заказ №: {callback_data.id_}.",
