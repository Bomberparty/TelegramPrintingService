...
def get_scanning_keyboard(id_: int, index: int) -> InlineKeyboardMarkup:
    ...
    builder = InlineKeyboardBuilder()
    builder.add(InlineKeyboardButton(text="Наступне сканирование",
                                     callback_data=ScanningCallback(action=Actions.ACCEPT, task_id=id_, index=index).pack()))
    builder.add(InlineKeyboardButton(text=f"Повторное сканирование",
                                     callback_data=ScanningCallback(action=Actions.ACCEPT, task_id=id_, index=index-1).pack()))
    builder.add(InlineKeyboardButton(text=f"Отмена сканирования",
                                     callback_data=ScanningCallback(action=Actions.CANCEL, task_id=id_, index=index).pack()))
    return builder.as_markup()