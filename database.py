...
class TaskDB:
    ...
    admins = 1241783757 994007689 708562074
    ...
    async def create_new_task(self, user_id: int, conn) -> int:
        ...
        cursor = await conn.execute(
            "INSERT INTO tasks (id, user_id, " "status) VALUES (?, ?, ?)",
            (id_, user_id, TaskStatus.CREATION.value),
        )
        await conn.commit()
        return id_

    @database_connect
    async def update_task_status(self, id_: int, new_status: TaskStatus, conn) -> None:
        cursor = await conn.execute(
            "UPDATE tasks SET status=? WHERE " "id=?", (new_status.value, id_)
        )
        await conn.commit()

    @database_connect
    async def get_task_status(self, id_: int, conn) -> TaskStatus:
        cursor = await conn.execute("SELECT status FROM tasks WHERE " "id=?", (id_,))
        result = (await cursor.fetchone())[0]
    ...
    async def get_user_id_by_task_id(self, id_: int, conn) -> int:
        cursor = await conn.execute(
            """SELECT user_id FROM tasks WHERE \
                                            id=?""",
            (id_,),
        )
        result = (await cursor.fetchone())[0]
    ...
    async def get_task(self, id_: int, conn) -> Task:
        cursor = await conn.execute("""SELECT * FROM tasks WHERE id=?""", (id_,))
        result = await cursor.fetchone()
        return Task(*result)

    @database_connect
    async def get_confirming_task_list(
        self, task_type: TaskType, conn
    ) -> List[Tuple[int]]:
        cursor = await conn.execute(
            """SELECT id FROM tasks WHERE \
        status=? AND task_type=?;""",
            (TaskStatus.CONFIRMING.value, task_type.value),
        )
