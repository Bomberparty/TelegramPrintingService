...
from keyboards.callbacks import Actions, AdminScanTaskCallback, ScanningCallback
...
@router.callback_query(
    and_f(
        AdminScanTaskCallback.filter(F.action == Actions.ACCEPT),
        AdminScanTaskStatusFilter(TaskStatus.CONFIRMING),
    )
)
async def accept_task(callback: CallbackQuery, callback_data: AdminScanTaskCallback):
    task = await TaskDB().get_task(callback_data.task_id)
    await callback.message.edit_text(
        f"Сканирование заказа № {task.id_} \n" f"Страница №1/{task.number_of_copies}",
        reply_markup=get_scanning_keyboard(task.id_, 1),
    )
...
async def scan_page(callback: CallbackQuery, callback_data: ScanningCallback):
    ...
    if task.number_of_copies == callback_data.index:
        await bot.send_message(
            task.user_id, f"Сканированик №{task.id_} завершилось. " f"Приходите ещё"
        )
        await callback.message.edit_text(
            f"Сканирование заказа №{task.id_} " f"завершилось.", reply_markup=None
        )
        return
