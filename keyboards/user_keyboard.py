...
def get_choosing_task_keyboard() -> ReplyKeyboardMarkup:
    ...
    keyboard = ReplyKeyboardMarkup(
        keyboard=kb, resize_keyboard=True, input_field_placeholder="Выберите тип заказа"
    )
    return keyboard
...
def get_printing_method_kb() -> ReplyKeyboardMarkup:
    kb = [
        [
            KeyboardButton(text="Одностороння печать"),
            KeyboardButton(text="Двусторонняя печать"),
        ],
        [KeyboardButton(text="Отмена"), KeyboardButton(text="Назад")],
    ]
...
def get_simple_keyboard() -> ReplyKeyboardMarkup:
    kb = [[KeyboardButton(text="Отмена"), KeyboardButton(text="Назад")]]
    keyboard = ReplyKeyboardMarkup(keyboard=kb, resize_keyboard=True)
    return keyboard
    ...
    builder.add(
        InlineKeyboardButton(
            ...
            callback_data=CardCallback(
                action=Actions.ACCEPT, id_=id_, task_type=task_type, task_id=task_id
            ).pack(),
        )
    )
    builder.add(
        InlineKeyboardButton(
            text="Отменить",
            callback_data=CardCallback(
                action=Actions.CANCEL, id_=id_, task_type=task_type, task_id=task_id
            ).pack(),
        )
    )
