...
@router.callback_query(
    and_f(
        PrintTaskCompletingCallback.filter(F.action == Actions.ACCEPT),
        TaskCompletingStatusFilter(TaskStatus.PENDING),
    )
)
async def task_complete(
    callback: CallbackQuery, callback_data: PrintTaskCompletingCallback
):
    task = await TaskDB().get_task(callback_data.task_id)
...
@router.callback_query(
    and_f(
        PrintTaskCompletingCallback.filter(F.action == Actions.CANCEL),
        TaskCompletingStatusFilter(TaskStatus.PENDING),
    )
)
async def task_failed(
    ...
    await bot.send_message(
        task.user_id,
        "Печать успешно провалилась. Свяжитесь" " с админами из 254 комнаты",
    )
